...
hepatocytes = scater::runPCA(hepatocytes, ncomponents = 7, scale_features = T)
...
data_attr = merge_arch_dist(arch_data = average_pch_fits(arc), data = PCs4arch, 
...
```{r}
# fit a polytope once to make randomisation test faster
...
p_pca = plot_arc(arch_data = arc_1, data = PCs4arch, 
                 which_dimensions = 1:3, line_size = 1.5, 
                 data_lab = logcounts(hepatocytes["Alb",]),
                 text_size = 60, data_size = 6) 
