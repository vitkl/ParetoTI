...
```{r normal_paa, fig.width=14, fig.height=5, eval=FALSE, echo=FALSE}
####### ---- Normal model - prior predictive simulation ---- #######
...
m$arc = arc
plot(m$model)
prior_draws = mcmc(m$model, n_samples = 1, warmup = 0, thin = 1)
prior_data = calculate(m$prior_data, prior_draws)
prior_archetypes = calculate(m$archetypes_mean, prior_draws)
arc_norm_free = copy(arc_data_free)
arc_norm_free$XC = t(array(t(as.matrix(prior_archetypes[[1]])), dim = c(dim(arc), nrow(prior_archetypes[[1]])))[,,1])
plot_data = t(array(t(as.matrix(prior_data[[1]])), dim = c(dim(data), nrow(prior_data[[1]])))[,,1])
rownames(arc_norm_free$XC) = NULL
...
cowplot::plot_grid(
  plot_arc(arc_norm_free, data, data_size = 0.1,
    ...
    ggtitle("Data"),
  plot_arc(arc_norm_free, plot_data, data_size = 0.1,
           geom = ggplot2::geom_bin2d) +
    #xlim(-10, 150) + ylim(-10, 120) +
